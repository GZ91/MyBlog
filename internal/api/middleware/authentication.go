...
func (m *Middleware) Authentication(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil && err != http.ErrNoCookie {
			...
			m.logger.Error("authorization", zap.Error(err))
		...
		if err == http.ErrNoCookie {
			...
			var ok bool
			userID, ok, err = m.validGetAuthentication(cookie.Value)
			if err != nil {
...
func (m *Middleware) validGetAuthentication(tokenString string) (string, bool, error) {
	claims := &Claims{}
	token, err := jwt.ParseWithClaims(tokenString, claims,
		func(t *jwt.Token) (interface{}, error) {
			if _, ok := t.Method.(*jwt.SigningMethodHMAC); !ok {
				strErr := fmt.Sprintf("unexpected signing method: %v", t.Header["alg"])
				m.logger.Error(strErr)
