...
func (m *Middleware) Compress(h http.Handler) http.Handler {
	compMid := func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.Contains(r.Header.Get("Content-Encoding"), "gzip") {
			m.logger.Info("middleware_Compress", zap.String("status", "decompression added - gzip"))
			...
			if err != nil {
				...
				m.logger.Error("middleware_Compress", zap.String("error", err.Error()))
				return
			}
			defer reader.Close()
			r.Body = reader
		...
		if AcceptEncoding && AcceptEncodingType {
			m.logger.Info("middleware_Compress", zap.String("status", "compression added - gzip"))
